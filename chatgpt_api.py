...
# ------------------- CONFIGURATION -------------------
OUTPUT_FILE = 'data/raw_citations_openai.csv'
...
# ------------------- OPENAI API CALL ---------------------
def get_citations(prompt, country, prompt_type, max_retries=3, retry_delay=2):
...
def main():
    ...
    print(f"Starting citation generation with OpenAI {MODEL_NAME}")
    ...
    for i, task in enumerate(ALL_TASKS[start_index:], start_index + 1):
        ...
        if citation_text:
            ...
            filename = f"openai_{country.replace(' ', '_')}_{prompt_type.lower()}.txt"
